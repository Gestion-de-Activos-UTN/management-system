...
import type { AssetBusinessFields, AssetIdentification } from './schema'
...
export function identifyAsset(id: string, identification: AssetIdentification) {
  return httpClient.patch<Asset>(`/api/v1/assets/${id}/identify`, identification)
}

// Vuelve el asset a 'pending' sin tocar alias/location/criticality/owner/confirmed_type/
// authorization_status (quedan guardados para prepopular si se vuelve a identificar, pero dejan
// de contar en cálculos que filtran por identified/identification_status — ver
// domain/inventories/createInventorySnapshot.ts).
export function unidentifyAsset(id: string) {
  return httpClient.patch<Asset>(`/api/v1/assets/${id}/unidentify`, {})
}
