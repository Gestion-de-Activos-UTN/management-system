 ...
 * app/portal/(protected)/layout.tsx). Selection is stored in Zustand's ui-store and
 * read from there by modules/assets/hooks/use-assets.ts's queryKey. `null` means
 * "All Offices" (no filter) — the default whenever there's more than one office;
 * with exactly one office, that office is selected automatically since there's no
 * real choice to make.
 */
export function OfficeSelector() {
  ...
  }, [selectedOfficeId, officeIds, setSelectedOfficeId]);
