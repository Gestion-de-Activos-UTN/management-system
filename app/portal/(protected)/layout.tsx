...
import {
  ...
  isEffectiveOrgAdmin,
  roleLabel,
} from '@/modules/auth/hooks/use-tenant-context'
import { OfficeSelector } from '@/modules/offices/components/OfficeSelector'
import { ScanResultMonitor } from '@/modules/scan-reports/components/ScanResultMonitor'
...
function PortalProtectedLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {tenantContext.data?.organizationId && (
        <ScanResultMonitor
          organizationId={tenantContext.data.organizationId}
